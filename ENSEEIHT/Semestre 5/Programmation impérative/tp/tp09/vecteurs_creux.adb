...
package body Vecteurs_Creux is
    ...
    begin
        while not Est_Nul(V) and then V.All.Indice <= Indice loop
            if V.All.Indice = Indice then
                return V.All.Valeur;
            end if;
        end loop;
	...
	procedure Modifier (V : in out T_Vecteur_Creux ;
				       Indice : in Integer ;
                     Valeur : in Float ) is
        curseur, precedent  : T_Vecteur_Creux;
    ...
    begin
        if Valeur /= 0.0 then
            inserer:= false;
            curseur := v;
            precedent:= new T_Cellule;

            while not Est_Nul(curseur) and then curseur.All.Indice <= Indice loop
                if curseur.All.Indice = Indice then
            ...
            if not inserer then
                new_V:= new T_Cellule;
                new_V.all.Indice:=Indice;
                new_V.all.Valeur:=Valeur;

                if not Est_Nul(curseur) then
                    new_V.all.Suivant:=curseur.All.Suivant;
                    curseur.All.Suivant:=new_V;
                else
                    new_V.all.Suivant:=null;
                    precedent.All.Suivant:=new_V;
                end if;
