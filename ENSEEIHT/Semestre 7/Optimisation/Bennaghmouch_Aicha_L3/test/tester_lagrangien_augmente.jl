...
function tester_lagrangien_augmente(afficher::Bool,Lagrangien_Augmente::Function)
	...
	@testset "Lagrangien augmenté  " begin 					
		@testset "Avec $algo" for algo in algos									
			# le cas de test 1
			xmin,fxmin,flag,nbiters,muks,lambdaks = Lagrangien_Augmente(algo,fct1,contrainte1,grad_fct1,hess_fct1,grad_contrainte1,
			...
			if (afficher)
				...
				println("L'évolution de muk", muks)
				println("L'évolution de lambdak", lambdaks)
			...
			# le cas de test 3
			xmin,fxmin,flag,nbiters,muks,lambdaks = Lagrangien_Augmente(algo,fct2,contrainte2,grad_fct2,hess_fct2,grad_contrainte2,
			...
			hess_contrainte2,pts2.x04,options)
			if (afficher)
				afficher_resultats("Lagrangien augmenté avec "*algo,"fonction 2" ,"x04","tho", tho,xmin,fxmin,flag,sol_fct2_augm,nbiters)
				println("L'évolution de muk", muks)
				println("L'évolution de lambdak", lambdaks)
			end
			@test xmin ≈ sol_fct2_augm atol=tol_erreur
