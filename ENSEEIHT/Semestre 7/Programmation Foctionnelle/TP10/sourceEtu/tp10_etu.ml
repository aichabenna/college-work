...
let rec parse_P : (char, prog) parser = fun flux ->
  (
    (*p_begin >>= fun () -> parse_I >>= fun i -> p_end >>= fun () -> return i*)
    (p_begin *> parse_I *> p_end >>= fun (((), i), ()) -> return i)
  ) flux
and parse_I : (char, inst) parser = fun flux ->
  (
    (return []) ++ (parse_C *> p_ptvirg *> parse_I >>= fun ((c , ()), i) -> return (c::i) )
  ) flux
and parse_C : (char, cmd) parser = fun flux ->
  (
    (p_repeat *> p_entier *> parse_P >>= fun (((),n), p) -> return (Repeat(n, p)))
    ++
    (p_move *> p_entier >>= fun ((),n) -> return (Move n) )
    ++
    (p_turn *> p_entier >>= fun ((),n) -> return (Turn n) )
    ++ 
    (p_on >>= fun () -> return On)
    ++
...
(* Parser pour les ident                               *)
let is_lettre c = c >= 'a' && c<= 'z';;
let p_lettre = ptest is_chiffre;; 
...
let rec parse_P : (char, prog) parser = fun flux ->
  (
    (*p_begin >>= fun () -> parse_I >>= fun i -> p_end >>= fun () -> return i*)
    (p_begin *> parse_D *> parse_I *> p_end >>= fun (((), i), ()) -> return i)
  ) flux
and parse_I : (char, inst) parser = fun flux ->
  (
    (return []) ++ (parse_C *> p_ptvirg *> parse_I >>= fun ((c , ()), i) -> return (c::i) )
  ) flux
and parse_S : (char, decls) parser = fun flux ->
  (
    zero
  ) flux
and parse_D : (char, decl) parser = fun flux ->
  (
    zero
  ) flux
and parse_C : (char, cmd) parser = fun flux ->
