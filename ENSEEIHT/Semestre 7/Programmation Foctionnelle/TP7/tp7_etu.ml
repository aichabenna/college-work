...
module FreeFall (F : Frame) =
  struct
    ...
    let run : etat -> etat Flux.t = fun (pos0, v0) ->
      let acceleration = Flux.constant (0., -. g) in
      let vitesse      = integre F.dt v0 acceleration in
      let position     = integre F.dt pos0 vitesse in
...
module Free = FreeFall (Test)
module Draw = Drawing (Test)

let exo2() = 
  let vitesse0 =(20., 25.) in
  let pos0 =(200., 300.) in
  let etat0 = (pos0, vitesse0) in
  Draw.draw (Free.run etat0);;
...
struct
  let rec unless flux cond f_cond =
      Tick (lazy (
                ...
                | None        -> None
                | Some (t, q) -> if cond t then Flux.uncons (f_cond t) 
                                else Some (t, unless q cond f_cond)
        ))
end

let contact_1d (inf, sup) z dz =
  (z <= inf && dz < 0.) || (z >= sup && dz > 0.)

let contact_x = contact_1d Test.box_x

let contact_y = contact_1d Test.box_y
