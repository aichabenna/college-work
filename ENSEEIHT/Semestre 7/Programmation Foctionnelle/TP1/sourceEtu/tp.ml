...
let coeff_directeur (x1,y1) (x2,y2) = 
   if abs (x1 -. x2) <= 1e-6 then raise DivisionZero
   (y2 -. y1) /. (x2 -. x1)
...
*)
(* on pose x = n+3 et on obtient: ux = u(x-2) + u(x-3)*)
let padovan n = 
   let rec aux n u0 u1 u2 =
      if n=0 | n=1 then 0
      else if n=2 then 1
