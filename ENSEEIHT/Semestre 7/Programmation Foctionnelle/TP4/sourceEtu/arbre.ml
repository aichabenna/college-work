...
(******************************************************************************)
let rec liste_elements (Noeud(b, lb)) = 
   match b,lb with
  | false,[] -> []
  | true,[] -> [[]]
  | _,(c,arb)::qlb -> (List.map (fun l-> c::l) (liste_elements arb))
                          @(liste_elements (Noeud(b,qlb)))

let rec normalisation (Noeud(b, lb)) = match b,lb with
  | false,[] -> Noeud(false,[])
  | b,lb -> 
    let liste = List.map (fun (c,arbre) -> c,normalisation arbre) lb in
    Noeud(b,List.filter (fun (c,arb) -> arb <> Noeud(false,[])) liste)