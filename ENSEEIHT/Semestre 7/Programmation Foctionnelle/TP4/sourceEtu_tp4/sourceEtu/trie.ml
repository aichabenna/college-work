...
let trie_dico trie = 
  match trie with
| Trie (arbre, _, recomp) -> List.map recomp (parcours_arbre arbre)
...
let affiche (p: 'a -> unit) (Trie(arbre,deco,reco)) =
  let seq_liste = parcours_arbre arbre in
  let mot_liste = List.map reco seq_liste in
  List.iter p mot_liste
